...
from lab_mgr.link_model_admin import LinkModelAdminFormSet
from lab_mgr.link_form_admin import LinkFormAdminFormset
...
class MyModelAdmin(admin.ModelAdmin):
    ...
    def filter_objects(self, fk_model, LinkObjClass, obj):
        ...
        if  fk_model == Product:
            link_objs = LinkObjClass.objects.filter(Product = obj)
        if  fk_model == IP_Type:
            link_objs = LinkObjClass.objects.filter(IP_Type = obj)
        if  fk_model == Configuration_Type:
            link_objs = LinkObjClass.objects.filter(Configuration_Type = obj)
        if  fk_model == lab_device_item:
            link_objs = LinkObjClass.objects.filter(lab_device_item = obj)

        if  fk_model == lab_ip:
            link_objs = LinkObjClass.objects.filter(lab_ip = obj)
        if  fk_model == lab_configuration:
            link_objs = LinkObjClass.objects.filter(lab_configuration = obj)
        if  fk_model == Platform:
            link_objs = LinkObjClass.objects.filter(Platform = obj)
        if  fk_model == UE:
